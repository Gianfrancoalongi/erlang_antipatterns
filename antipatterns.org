...
Seq is as the name describes sequential, and is not intended for recursive computations.
...
It is very simple to realize that the end condition is that we recieved "die".
...
we have more code, but it is very easy to extend, just add another function in the appropriate place 
