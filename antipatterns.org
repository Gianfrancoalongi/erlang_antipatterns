...
further into the rec style (a cousin of seq that allows for an elegant recursive programming style).
...
However, this is not ideal as it makes us believe that all data is needed for each function, and also makes 
...
#+BEGIN_EXAMPLE
seq([{fun get_f_p/1, fun f_p/1, fun update_f_p/1},
     {fun get_f_m/1, fun f_m/1, fun update_f_m/1},
     {fun get_f_n/1, fun f_n/1, fun update_f_n/1},
    ], Data).

seq([],Data) -> Data;
seq([{Get,F, Update}|T],Data) ->
