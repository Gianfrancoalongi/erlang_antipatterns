...
We can still utilize the specific $get_{f_i}/update_{f_i}$ functions to work with the data.
...
The implicit assumption here is that every function returns {ok,Data} or {error,Reason} as stated
